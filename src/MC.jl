...
export RV, ArrayRV, sample, hist, ProductSample, ParallelSample
...
function sample_rvs(ex::NExpr, ::ProductSample)
    ...
    product(variates...), n_out
end

function sample_rvs(ex::NExpr, method::ParallelSample)
    variates = map(x -> rand(sample(x), method.n), ex.rvs)
    @debug variates
    n_out = method.n
    @debug n_out
    # reshape(collect(product(variates...)), n_out), n_out
...
# Histogram for comparing samples.
hist(x) = sort(Dict(counter(x)))
